...
type Props = {
  ...
  errorMessage?: string;
  onChange?: (e: React.ChangeEvent<HTMLInputElement>) => void; // react-hook-form compatible
  onBlur?: (e?: any) => void;
  name?: string;
  value?: string;
  disabled?: boolean;
...
const CustomInputDate: React.FC<Props> = ({
  ...
  return (
      <div className="w-full relative flex flex-col gap-[6px] mb-[16px]">
        ...
        <IonModal isOpen={isOpen} onDidDismiss={() => setIsOpen(false)} keepContentsMounted={true}>
          <IonDatetime
              id={name}
              presentation="date"
              value={value}
        ...
        {errorMessage && (
            <p className="text-red-500 text-[12px] font-paragraph">{errorMessage}</p>
        )}
      </div>
  );
};
