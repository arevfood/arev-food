...
const ContentsSettingsChangePassword: React.FC = () => {
    ...
    const onSubmit: SubmitHandler<inputProps> = async (data) => {
        try {
            ...
            showToast({header: "Password Updated", message: "Your password has been changed.", type: "success"});
            router.push("/setting");
        } catch (error: any) {
            if (error.code === 'auth/requires-recent-login') {
                showToast({header: "Update Failed", message: "Please log in again to change.", type: "error"});
            } else {
                showToast({header: "Update Failed", message: "Unable to change password. Try again.", type: "error"});
            }
        } finally {
            setValue('password', '');
